...
public class ShoppingWomenContrl {
	...
	public String show(Model model,@PageableDefault Pageable pageable, @ModelAttribute("filter") ItemOfClothingFilter filter, Principal principal, @PathVariable String itemType){
		...
		List <Integer> colorList = filter.getColorId();
		...
		for (Integer integer : colorList){
			Color color = colorService.findOne(integer);
			colors.add(color);
		}
		model.addAttribute("colors", colors);
		List <Brand> brands = new ArrayList<>();
		List <Size> sizes = new ArrayList<>();
		List <ItemOfClothing> items = itemOfClothingService.findAll();
		for (ItemOfClothing item : items){
			for (Color color: colors){
				if(item.getColor().getColor().equals(color.getColor())){
